...
RAS_REPOS="ras-party" "ras-secure-message" "ras-frontstage" "ras-collection-instrument" "django-oauth2-test" "ras-rm-auth-service" "respondent-home-ui" "response-operations-ui" "response-operations-social-ui" "sdc-uaa"
RM_REPOS="rm-sample-service" "rm-case-service" "rm-action-service" "rm-actionexporter-service" "iac-service" "rm-sdx-gateway" "rm-collection-exercise-service" "rm-survey-service" "rm-notify-gateway" "rm-comms-template-service"
...
build: clone
	...
	@ for r in ${REPOS}; do \
		...
		if [ ! -e ${RASRM_HOME}/$${r}/pom.xml ]; then \
			...
			mvn clean install -Dmaven.test.skip=true -DskipITs -f ${RASRM_HOME}/$${r}; \
		fi; echo ""; \
	done
